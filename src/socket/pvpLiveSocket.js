...
async function executarTurno(io, duelId, chave, acao, foiAutomatico = false) {
  ...
  const payloadTurno = {
    ...
    logStatus,
    statusA: duelo.statusEffects.A.map((s) => ({ key: s.key, remainingTurns: s.remainingTurns })),
    statusB: duelo.statusEffects.B.map((s) => ({ key: s.key, remainingTurns: s.remainingTurns })),
